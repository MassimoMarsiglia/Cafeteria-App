...
export default function HomeScreen() {
  ...
  const {
    ...
    error: menuError,
    refetch: refetchMenu,
  } = useTodaysMenu(selectedCanteenId);
  ...
  useEffect(() => {
    if (canteens && canteens.length > 0 && !selectedCanteenId) {
      ...
      console.log(
        'Available canteens:',
        canteens.map(c => ({ ID: c.ID, name: c.name })),
      );

      // Look for HTW canteen with multiple search criteria
      const htwCanteen = canteens.find(
        canteen =>
      ...
      console.log(
        'HTW Canteen found:',
        htwCanteen ? { ID: htwCanteen.ID, name: htwCanteen.name } : 'Not found',
      );
    ...
    >
      ...
      >
        ...
        {__DEV__ && (
          ...
          >
            Debug: Canteen ID: {selectedCanteenId || 'none'} | Menu items:{' '}
            {todaysMenu?.length || 0} | Total meals: {allMeals.length}
          </ThemedText>
        )}
        ...
        {!loading && !error && (!allMeals || allMeals.length === 0) && (
          <ThemedView style={styles.centerContainer}>
            ...
            <ThemedText style={styles.emptyText}>
              Heute sind keine Gerichte verfügbar
            </ThemedText>
          </ThemedView>
        )}
